...
function ClimateList() {
  ...
  const fetchItems = async () => {
    setLoading(true)
    try {
      const res = await request.get(`/content/climate?page=${currentPage}&page_size=12`)
        ...
        {items.map((item) => (
          <Col xs={24} sm={12} md={8} key={item.id}>
            <Card
              ...
              onClick={() => navigate(`/climate/${item.id}`)}
              cover={
                <img
                  alt={item.title}
                  src={item.cover_image || 'https://trae-api-cn.mchost.guru/api/ide/v1/text_to_image?prompt=climate%20change%20ocean%20warming%20coral%20reef&image_size=square'}
